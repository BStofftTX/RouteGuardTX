...
4. `ProviderProxyAdapter` sends normalized constraints to a protected RouteGuardTX API.
...
- RouteGuardTX API: credential isolation, normalization, provider selection, audit logs.
...
Provider credentials must not be shipped in browser/mobile source. The frontend calls a RouteGuardTX-controlled API, which holds credentials in a protected secret store and returns a normalized result.
