...
import { DEFAULT_CONSTRAINTS, validateRequest, type RouteRequest } from '../src/domain/route';
...
const request: RouteRequest = { origin: 'Junction, TX', destination: 'Phoenix, AZ', profile: 'heavy-load', constraints: DEFAULT_CONSTRAINTS };

describe('route request', () => {
  it('validates required places', () => expect(validateRequest({ ...request, origin: '' })).toContain('Enter an origin.'));
  it('creates deterministic offline candidates', async () => {
    const adapter = new MockRoutingAdapter();
    expect(await adapter.compute(request)).toEqual(await adapter.compute(request));
  });
  it('builds supported Google Maps URL fields', () => {
    const url = googleMapsUrl(request);
    expect(url).toContain('api=1'); expect(url).toContain('travelmode=driving');
  });
  it('builds an Apple driving link', () => expect(appleMapsUrl(request)).toContain('dirflg=d'));
  ...
  it('uses the provider proxy when configured', () => {
    const session = createRoutingSession({ VITE_ROUTEGUARD_PROXY_URL: 'http://localhost:8787/api/routes' });
