...
export default function App() {
  ...
  const [constraints, setConstraints] = useState<VehicleConstraints>(DEFAULT_CONSTRAINTS);
  const [routes, setRoutes] = useState<RouteOption[]>([]);
  const [errors, setErrors] = useState<string[]>([]);
  const [loading, setLoading] = useState(false);
  const request = useMemo<RouteRequest>(() => ({ origin, destination, profile, constraints }), [origin, destination, profile, constraints]);
  const setConstraint = <K extends keyof VehicleConstraints>(key: K, value: VehicleConstraints[K]) => setConstraints(current => ({ ...current, [key]: value }));
  ...
  return <main>
    ...
    <section className="planner">
      <div className="panel route-inputs"><div className="section-title"><span>01</span><h2>Trip</h2></div>
      ...
      <div className="panel profile-panel"><div className="section-title"><span>02</span><h2>Route profile</h2></div>
        <div className="profiles">{PROFILES.map(item => <button key={item.id} className={profile === item.id ? 'profile selected' : 'profile'} onClick={() => setProfile(item.id)}><b>{item.icon} {item.label}</b><small>{item.description}</small></button>)}</div>
      </div>

      <div className="panel constraints"><div className="section-title"><span>03</span><h2>Vehicle & constraints</h2></div>
        <div className="measurements">
          <label>Height (ft)<input type="number" step="0.1" value={constraints.heightFt} onChange={e => setConstraint('heightFt', Number(e.target.value))}/></label>
          <label>Weight (lb)<input type="number" value={constraints.weightLbs} onChange={e => setConstraint('weightLbs', Number(e.target.value))}/></label>
          <label>Length (ft)<input type="number" value={constraints.lengthFt} onChange={e => setConstraint('lengthFt', Number(e.target.value))}/></label>
          <label>Width (ft)<input type="number" step="0.1" value={constraints.widthFt} onChange={e => setConstraint('widthFt', Number(e.target.value))}/></label>
        </div>
        <div className="toggles">
          <Toggle label="Avoid sharp turns" checked={constraints.avoidSharpTurns} onChange={v => setConstraint('avoidSharpTurns', v)}/>
          <Toggle label="Require turnaround room" checked={constraints.requireTurnaround} onChange={v => setConstraint('requireTurnaround', v)}/>
          <Toggle label="Avoid low clearances" checked={constraints.avoidLowClearance} onChange={v => setConstraint('avoidLowClearance', v)}/>
          <Toggle label="Avoid weight-restricted bridges" checked={constraints.avoidWeightRestrictedBridges} onChange={v => setConstraint('avoidWeightRestrictedBridges', v)}/>
          <Toggle label="Avoid highways" checked={constraints.avoidHighways} onChange={v => setConstraint('avoidHighways', v)}/>
          <Toggle label="Avoid tolls" checked={constraints.avoidTolls} onChange={v => setConstraint('avoidTolls', v)}/>
        </div>
        {errors.length > 0 && <div className="errors">{errors.map(error => <p key={error}>{error}</p>)}</div>}
        <button className="plan" onClick={plan} disabled={loading}>{loading ? 'Comparing…' : 'Compare route candidates'}</button>
      </div>
    </section>
