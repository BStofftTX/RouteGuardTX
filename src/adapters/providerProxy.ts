...
interface ProxyResponse { routes: RouteOption[]; }
...
export class ProviderProxyAdapter implements RoutingAdapter {
  ...
  async compute(request: RouteRequest): Promise<RouteOption[]> {
    ...
    if (!response.ok) throw new Error(`Live routing service failed (${response.status}).`);
...
export const feetToMeters = (feet: number) => Number((feet * 0.3048).toFixed(3));
export const poundsToKilograms = (pounds: number) => Math.round(pounds * 0.45359237);
