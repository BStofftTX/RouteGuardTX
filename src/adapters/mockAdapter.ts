...
export class MockRoutingAdapter implements RoutingAdapter {
  ...
  async compute(request: RouteRequest): Promise<RouteOption[]> {
    ...
    const heavy = request.profile === 'heavy-load' || request.profile === 'towing-rv';
    const scenic = request.profile === 'scenic' || request.profile === 'motorcycle';
    return [
      {
        id: 'recommended',
        name: heavy ? 'Restriction-aware candidate' : scenic ? 'Profile-balanced candidate' : 'Fastest candidate',
